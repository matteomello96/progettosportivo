...
public class modellidettagli extends AbstractTableModel {
	...
	private String[] tableHeaders = {"Disciplina","Livello","Confermato","Annullato","Modificato","Giorno","Orario"};
	@Override
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		...
		else if(columnIndex==4) return d.getModifica(); 
		else if(columnIndex==5) return d.getGiorno();
		else if(columnIndex==6) return d.getOrario();
		  return null;
    ...
    public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
        ...
        if(columnIndex==4) dati.get(rowIndex).setModifica((int)aValue);
        if(columnIndex==5) dati.get(rowIndex).setGiorno(aValue.toString());
        if(columnIndex==6) dati.get(rowIndex).setOrario(aValue.toString());
